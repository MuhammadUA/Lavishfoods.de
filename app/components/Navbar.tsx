...
export default function Navbar() {
  ...
  return (
    <>
      {/* Top info bar */}
      <div className="fixed top-0 left-0 right-0 z-50 bg-[#030810] border-b border-white/[0.08] py-2 px-4 md:px-8">
        <div className="max-w-7xl mx-auto flex items-center justify-between text-[11px] font-mono tracking-wide">
          <div className="flex items-center gap-3 text-slate-400">
            <a href={`tel:${PHONE_DE}`} className="hover:text-white transition-colors flex items-center gap-1.5">
              <span>🇩🇪</span><span>{PHONE_DE}</span>
            </a>
            <span className="text-white/10 hidden sm:block">·</span>
            <a href={`tel:${PHONE_US}`} className="hover:text-white transition-colors hidden sm:flex items-center gap-1.5">
              <span>🇺🇸</span><span>{PHONE_US}</span>
            </a>
          ...
          <div className="hidden md:flex items-center gap-2 text-slate-600 tracking-[0.18em] uppercase text-[10px]">
            ISO 22000 <span className="text-white/10">·</span> HACCP <span className="text-white/10">·</span> HALAL <span className="text-white/10">·</span> SGS
          </div>
          <div className="flex items-center gap-3">
            <span className="hidden sm:flex items-center gap-1.5 border border-white/[0.08] px-2.5 py-0.5 text-[9px] font-mono tracking-[0.2em] uppercase text-slate-500">
              🇩🇪 Registered Germany
            </span>
            <a href={WHATSAPP} target="_blank" rel="noopener noreferrer" className="text-slate-400 hover:text-[#C9A84C] transition-colors" aria-label="WhatsApp">
      ...
      <header className={`fixed top-[33px] left-0 right-0 z-40 transition-all duration-300 ${
        scrolled ? "bg-[#060d1b]/96 backdrop-blur-md border-b border-white/[0.08]" : "bg-transparent"
      }`}>
        <div className="max-w-7xl mx-auto px-4 md:px-8 h-16 flex items-center justify-between">
          {/* Logo + wordmark */}
          <Link href="/" className="flex items-center gap-3">
            <Image src={LOGO} alt="Lavish Foods" width={40} height={40} className="rounded-full" unoptimized />
          ...
          <nav className="hidden md:flex items-center gap-8">
            {navLinks.map((l) => (
              <Link
                key={l.href}
                href={l.href}
                className="text-slate-400 hover:text-white text-[11px] tracking-[0.18em] uppercase font-medium transition-colors"
              >
                {l.label}
              </Link>
            ))}
          ...
          <div className="hidden md:flex items-center gap-3">
            <a
              ...
              className="bg-[#C9A84C] text-[#060d1b] text-[10px] font-bold tracking-[0.25em] uppercase px-5 py-2 hover:bg-[#D4AF57] transition-colors"
            >
              Get Quote
            </a>
          </div>
        ...
        {menuOpen && (
          <div className="md:hidden bg-[#060d1b]/98 border-t border-white/[0.08] px-6 py-6 space-y-4">
            ...
            <a
              href={`mailto:${EMAIL}`}
              className="block text-center border border-white/[0.15] text-slate-300 text-xs font-semibold tracking-[0.25em] uppercase py-3 mt-4 hover:bg-white/5 transition-colors"
            >
              {EMAIL}
            </a>
          </div>
